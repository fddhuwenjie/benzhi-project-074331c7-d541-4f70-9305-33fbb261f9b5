...
func replayFreeze(a domain.Aggregate, request string, payload any) (domain.Aggregate, bool, error) {
	rec, ok := a.Idempotency[request]
	if !ok {
		return domain.Aggregate{}, false, nil
	}
	if rec.Fingerprint != fingerprint(payload) {
		return a, true, domain.ErrIdempotency
	}
	return a, true, nil
}
...
func (s *Service) FreezeWithPreflight(id, actor, request string, expected int64, preflightDigest string) (domain.Aggregate, error) {
	...
	if replay, ok, replayErr := replayFreeze(a, request, payload); ok {
