...
// CreateProjectContext 只在命令入口观察取消状态；底层提交仍由 CreateProject 执行。
func (s *Service) CreateProjectContext(ctx context.Context, in CreateInput) (domain.Aggregate, error) {
	if err := ctx.Err(); err != nil {
		return domain.Aggregate{}, err
	}
	return s.CreateProject(in)
}
